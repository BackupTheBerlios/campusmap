...
class Camera{
	...
	public void moveToInter(FVector to, Integer duration, Integer acceleration) {
		m_vPosTo			= to;
		m_vPosDir			= m_vPosTo.subtract(m_vPos);
	...
	public void step(Float directionInDegree, Float distance) {
		...
		FVector newPos = offsetCam.add(m_vPos);
		if (applet.groundPlane.check(newPos).isZero() || applet.groundPlane.check(newPos).magnitudeSqr() < applet.groundPlane.check(m_vPos).magnitudeSqr()) {
			moveToNow(newPos);
			lookAtNow(offsetCam.add(m_vCenter));
		}
	}
	...
	public void lookAtInter(FVector to, Integer duration, Integer acceleration) {
		limitPovDistance();
		m_vCenterTo			= to;
		m_vCenterDir		= m_vCenterTo.subtract(m_vCenter);
	...
	public void rotateToNow(Float radiens, Float height) {
		//m_iRotateStartTime 	= 0;
		m_bCameraHasChanged = true;
		FVector povToPos = getPos().subtract(getCenter());
	...
	public void rotateToInter(Float to, Float height, Integer duration, Integer acceleration) {
		...
		m_vRotateDir		= getCenter().subtract(getPos());
	...
	public void yaw(float rad) {
		if (rad != 0.0f) {
			...
			FVector tempRotated = m_vLook.rotateAxis(-rad, m_vUp);
			if ( (tempRotated.e[0]*tempRotated.e[0] + tempRotated.e[1]*tempRotated.e[1]) > 0.2f || m_fRoll == 0.0f) {
				m_vLook.set(tempRotated);
				m_vCenter.set(m_vPos.add(m_vLook.multiply(m_vCenter.magnitude())));
				updateUp();
				m_bCameraHasChanged = true;
			}
		}
	...
	public void pitch(float rad) {
		if (rad != 0.0f) {
			...
			FVector tempRotated = m_vLook.rotateAxis(-rad, m_vLeft);
	...
	private void updateUp() {
		...
		helper.normalizeMe();
		helper = (helper.crossProduct(m_vUp)).normalize();
		m_vUp	= (m_vLook.crossProduct(helper)).normalize();
		
		
		// Calculate the roll if there is one
		if (m_fRoll != 0.0) {
			m_vUp = m_vUp.multiply((float)(Math.cos(m_fRoll)));
			m_vUp = m_vUp.add(helper.multiply((float)(Math.sin(m_fRoll)))); 
		}
	}
	
	//updates the three vectors describing the camera look direction
	private void updateValues() {
		m_vLook = (m_vCenter.subtract(m_vPos)).normalize();
		updateUp();
		m_vLeft = (m_vUp.crossProduct((m_vCenter.subtract(m_vPos)).normalize())).normalize();
	}
	...
	private void updatePos() {
		...
		if ( timePassed > m_iMoveDuration	||	m_vPosTo.equals(m_vPos) ) {
			...
			if(!m_bMoveBezier){
				...
				m_vPos = m_vPosTo.subtract(m_vPosDir.multiply(1 - timePassedOfTotal));
			} else {
	...
	private void updateCenter() {
		...
		if ( timePassed > m_iLookDuration	||	m_vCenterTo.equals(m_vCenter) ) {
			...
			m_vCenter = m_vCenterTo.subtract(m_vCenterDir.multiply(1- timePassedOfTotal));
	...
	public FVector birdView(FVector newTarget, float degree, int duration, float height) {
		FVector lookDirection = newTarget.subtract(getPos());
		...
		if (height == 0.0f) {
			...
			FVector addToTarget = newTarget.subtract(m_vPos).normalize().multiply(height/2);
			lookAtInter(newTarget.add(addToTarget), new Integer(duration), new Integer(1));
		}
		else {
...
//		Building targetBuilding = applet.objectManager.getBuildingByNumber(buildingNo);
	...
	public FVector flyIntoCircleView(FVector centerPoint, float radius, float degree, int duration) {
		...
		FVector dirFromCenterToPos = centerPoint.subtract(currentPos).normalize();
		FVector dirFromCenterToSide = dirFromCenterToPos.crossProduct(new FVector(0,0,1));
		FVector newCameraPos = centerPoint.add(dirFromCenterToSide.multiply(radius));
		//centerPoint.printMe();
		...
		return newCameraPos.subtract(centerPoint);
	}
	...
	public FVector getMousePointRay() {
		FVector mousePoint = getLook().multiply(m_fDistanceEyeMousePlane);
		mousePoint = mousePoint.add(getLeft().multiply( (float)((applet.mouseX - applet.width / 2) * -1) ) );
		mousePoint = mousePoint.add(getUp().multiply( (float)(applet.mouseY - applet.height / 2) ) );
		return mousePoint;
	}
	...
	public FVector getMouseGroundPlaneIntersection(boolean checkWithGroundPlane) {
		...
		if (mousePoint.e[2] != 0) {
			...
			if (multiplier > 0)
				intersection = getPos().add(mousePoint.multiply(multiplier));
		}
	...
	public FVector getScreenCenterGroundPlaneIntersection(boolean limit) {
		...
		if (centerPoint.e[2] != 0) {
			...
			if (multiplier > 0)
				intersection = getPos().add(centerPoint.multiply(multiplier));
		}
        // World-Borderline limitation
		if (limit)
	...
	public void limitPovDistance() {
		...
		if (currentPov.isZero())
			m_vCenter = m_vPos.add(m_vCenter.subtract(m_vPos).normalize().multiply(m_vPos.getZ()));
	...
	public void _rect(int _x, int _y, int _width, int _height) {
		...
		System.out.println(" " + " ");
		applet.pushMatrix();
		FVector translateTo = getLook().multiply(-m_fDistanceEyeMousePlane + m_fNearClip);
