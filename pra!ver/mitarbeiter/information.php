...
<div class="hintergrundstreifen">
		...
		<div class="titelGanz">
			...
			<?php echo "EINSTELLUNGEN F&Uuml;R ".$mitarbeiter->getName()." (".$mitarbeiter->getUsername().")"; ?>
		</div>
		...
		<div class="inhaltGanz">
			...
			<input type="hidden" name="update" value="5">
				<table border="0" cellspacing="0">
					...
					<tr>
						<td class="dick">Mitarbeiter für folgende Studiengänge:</td>
						<td>
							<?php
								if ($mitarbeiterStudiengaenge) {
								  while ($r = $mitarbeiterStudiengaenge->getNextRow()) {
      	...
      	<?php
      		...
      		if (!($result = $conn->executeQuery($select_query))) {
			    ...
			    if ($result->rowsCount() > 0) {
			    			echo "Zuständige Sachbearbeiter:<br";
								while ($r = $result->getNextRow()) {
								    echo $r[0].' | '.$r[1].' | <a href="'.$r[2].'">'.$r[2].'</a> | '.$r[3].'<br>';
								}
							} else {
								echo "Ihnen sind keine Studiengänge zugeordnet oder Ihren Studiengänge keine Praktikasachbearbeiter.";
							}
					}
      	?>
      	<br>
